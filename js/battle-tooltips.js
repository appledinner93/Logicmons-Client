...
var ability=serverPokemon.ability||pokemon&&pokemon.ability||serverPokemon.baseAbility;
...
tryItem=function tryItem(itemName){
...
tryAbility=function tryAbility(abilityName){
...
var valueString;
var roundedValue=this.value?Number(this.value.toFixed(2)):0;
var roundedMaxValue=this.maxValue?Number(this.maxValue.toFixed(2)):0;
if(this.isAccuracy){
...
var pokemon=this.battle.mySide.active[index];
...
var _side=this.battle.sides[_sideIndex];
...
var _side2=this.battle.sides[0];
...
if(elem){
var height=$(BattleTooltips.elem).height();
if(height>y){
y+=height+10;
if(ownHeight)y+=$(elem).height();else
y+=$(elem).parent().height();
$wrapper.css('top',y);
}
}
...
showMoveTooltip=function showMoveTooltip(move,isZ,pokemon,serverPokemon){
var text='';
...
var zMove=this.battle.dex.getMove(BattleTooltips.zMoveTable[item.zMoveType]);
...
text+='<h2>'+move.name+'<br />';var _this$getMoveType=


this.getMoveType(move,value),moveType=_this$getMoveType[0],category=_this$getMoveType[1];
...
var basePowers=[];for(var _i3=0;_i3<
foeActive.length;_i3++){var active=foeActive[_i3];
if(!active)continue;
...
showPokemonTooltip=function showPokemonTooltip(clientPokemon,serverPokemon,isActive){var _this3=this;
...
serverPokemon.moves;_i4<_serverPokemon$moves.length;_i4++){var _moveid=_serverPokemon$moves[_i4];
var move=Dex.getMove(_moveid);
var moveName='&#8226; '+move.name;
if(battlePokemon&&battlePokemon.moveTrack){for(var _i5=0,_battlePokemon$moveTr=
battlePokemon.moveTrack;_i5<_battlePokemon$moveTr.length;_i5++){var row=_battlePokemon$moveTr[_i5];
if(moveName===row[0]){
...
clientPokemon.moveTrack;_i6<_clientPokemon$moveTr.length;_i6++){var _row=_clientPokemon$moveTr[_i6];
text+=this.getPPUseText(_row)+'<br />';
}
if(clientPokemon.moveTrack.filter(function(_ref){var moveName=_ref[0];return(
...
var pokemon=clientPokemon||serverPokemon;
var isPowerTrick=clientPokemon&&clientPokemon.volatiles['powertrick'];for(var _i7=0,_Dex$statNamesExceptH=
Dex.statNamesExceptHP;_i7<_Dex$statNamesExceptH.length;_i7++){var statName=_Dex$statNamesExceptH[_i7];
...
if(this.battle.gen<=1){for(var _i8=0,_Dex$statNamesExceptH2=
Dex.statNamesExceptHP;_i8<_Dex$statNamesExceptH2.length;_i8++){var _statName=_Dex$statNamesExceptH2[_i8];
...
var species=Dex.getTemplate(clientPokemon?clientPokemon.getSpecies():serverPokemon.species).baseSpecies;
...
outer:for(var _i9=0,_this$battle$sides2=this.battle.sides;_i9<_this$battle$sides2.length;_i9++){var side=_this$battle$sides2[_i9];for(var _i10=0,_side$active2=
side.active;_i10<_side$active2.length;_i10++){var active=_side$active2[_i10];
if(active&&['Air Lock','Cloud Nine'].includes(active.ability)){
weather='';
break outer;
}
...
var allyActive=clientPokemon&&clientPokemon.side.active;
if(allyActive){for(var _i11=0;_i11<
allyActive.length;_i11++){var ally=allyActive[_i11];
...
var abilityName=ability==='plus'?'Plus':'Minus';for(var _i12=0;_i12<
_allyActive.length;_i12++){var _ally=_allyActive[_i12];
...
var isTransformed=clientPokemon&&clientPokemon.volatiles.transform;
...
if(!short){
var hasModifiedStat=false;for(var _i13=0,_Dex$statNamesExceptH3=
Dex.statNamesExceptHP;_i13<_Dex$statNamesExceptH3.length;_i13++){var statName=_Dex$statNamesExceptH3[_i13];
...
buf+='<p>';
}for(var _i14=0,_Dex$statNamesExceptH4=

Dex.statNamesExceptHP;_i14<_Dex$statNamesExceptH4.length;_i14++){var _statName2=_Dex$statNamesExceptH4[_i14];
...
ppUsed=function ppUsed(move,pokemon){for(var _i15=0,_pokemon$moveTrack=
pokemon.moveTrack;_i15<_pokemon$moveTrack.length;_i15++){var _ref2=_pokemon$moveTrack[_i15];var moveName=_ref2[0];var ppUsed=_ref2[1];
...
var baseSpe=pokemon.getTemplate().baseStats['spe'];
...
getMoveType=function getMoveType(move,value){
...
value.abilityModify(1.3,"Compound Eyes");for(var _i16=0,_pokemon$side$active=
pokemon.side.active;_i16<_pokemon$side$active.length;_i16++){var active=_pokemon$side$active[_i16];
...
var boostCount=0;for(var _i17=0,_Object$values=
Object.values(target.boosts);_i17<_Object$values.length;_i17++){var boost=_Object$values[_i17];
...
var _boostCount=0;for(var _i18=0,_Object$values2=
Object.values(pokemon.boosts);_i18<_Object$values2.length;_i18++){var _boost=_Object$values2[_i18];
...
var _basePower=40;
if(ppLeft===1)_basePower=200;else
if(ppLeft===2)_basePower=80;else
if(ppLeft===3)_basePower=60;else
if(ppLeft===4)_basePower=50;
value.set(_basePower);
}
...
var minRatio=serverPokemon.stats['spe']/maxSpe;
var maxRatio=serverPokemon.stats['spe']/minSpe;
var min;
var max;
...
var _min=Math.floor(25*_minSpe/serverPokemon.stats['spe'])||1;
if(_min>150)_min=150;
var _max=Math.floor(25*_maxSpe/serverPokemon.stats['spe'])||1;
...
if(isGKLK){
_basePower2=20;
if(targetWeight>=200)_basePower2=120;else
if(targetWeight>=100)_basePower2=100;else
if(targetWeight>=50)_basePower2=80;else
if(targetWeight>=25)_basePower2=60;else
if(targetWeight>=10)_basePower2=40;
}else{
_basePower2=40;
if(pokemonWeight>targetWeight*5)_basePower2=120;else
if(pokemonWeight>targetWeight*4)_basePower2=100;else
if(pokemonWeight>targetWeight*3)_basePower2=80;else
if(pokemonWeight>targetWeight*2)_basePower2=60;
}
...
var auraBoosted='';
var auraBroken=false;for(var _i19=0,_pokemon$side$active2=
pokemon.side.active;_i19<_pokemon$side$active2.length;_i19++){var ally=_pokemon$side$active2[_i19];
if(!ally||ally.fainted)continue;
var allyAbility=this.getAllyAbility(ally);
...
}for(var _i20=0,_pokemon$side$foe$act=
pokemon.side.foe.active;_i20<_pokemon$side$foe$act.length;_i20++){var foe=_pokemon$side$foe$act[_i20];
...
if(!types)types=this.getPokemonTypes(pokemon);for(var _i21=0,_types=
types;_i21<_types.length;_i21++){var curType=_types[_i21];
...
var species=clientPokemon.getSpecies()||serverPokemon&&serverPokemon.species||'';
var template=this.battle.dex.getTemplate(species);
...
var needsFourMoves=!['unown','ditto'].includes(template.id);
...
getStat=function getStat(stat,set,evOverride,natureOverride){
